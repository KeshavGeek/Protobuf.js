...
Protobuf.Decoder = Class.create({
  ...
  decode: function(url,urlOrData) {
    ...
    while (this.stream.length != 0) {
      ...
      var keshav_var = this.readKey();
      var type = keshav_var[0];
      var tag = keshav_var[1];
     ...
     switch (type){
	      ...
	      case Protobuf.WireType.LENGTH_DELIMITED:
			...
			if (field.type == 'string') {
			  ...
			  this.setField(ret, new Protobuf.Decoder(embeddedDescription).decode(data,false), field);
